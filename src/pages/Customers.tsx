...
import { useCustomers, type Order } from "@/hooks/useSupabase";
...
export default function Customers() {
  ...
  const tierCounts = useMemo(() => {
    ...
    return {
      ...
      platinum: customers.filter((c: any) => c.totalSpent >= 1000000).length,
      gold: customers.filter((c: any) => c.totalSpent >= 500000 && c.totalSpent < 1000000).length,
      silver: customers.filter((c: any) => c.totalSpent >= 200000 && c.totalSpent < 500000).length,
      bronze: customers.filter((c: any) => c.totalSpent < 200000).length,
    };
  }, [customers]);
  ...
  const totalStats = useMemo(() => {
    ...
    const totalOrders = customers.reduce((sum: number, c: any) => sum + c.totalOrders, 0);
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredCustomers.map((customer: any) => {
                      const tier = getCustomerTier(customer.totalSpent);
                      return (
                        <TableRow key={customer.user_id || customer.email} className="group">
                          <TableCell>
